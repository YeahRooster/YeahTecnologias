  ...
  if (newStatus === 'Cancelado' && oldStatus !== 'Cancelado') {
    ...
    if (productsContent.includes('(x')) {
      ...
      if (productNames.length === quantities.length) {
        ...
        console.warn(`⚠️ No se puede restituir stock exacto para pedido antiguo ${orderId}: formatos no coinciden`);
      }
    }
    ...
    if (itemsToRestore.length > 0) {
      ...
      for (const item of itemsToRestore) {
        ...
        if (productRow) {
          ...
          await productRow.save();
          console.log(`🔄 Stock restituido para ${item.name}: +${item.qty}`);
        }
      }
    }
  }
